...
- The built file carries a **Content-Security-Policy meta tag generated by the build**: `default-src 'none'`, the inline script pinned by sha256, `style-src 'unsafe-inline'`, `base-uri 'none'`, `form-action 'none'`. This is the zero-network rule above stated to the browser rather than only to the test suite. `style-src` stays open because the app sets `element.style` directly and hashes do not cover style attributes. A test recomputes the hash from the built file, so the policy cannot drift from the bundle.
